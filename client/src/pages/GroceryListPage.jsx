...
function GrocerySection({ section, items, checked, toggleCheck, handleRemoveItem }) {
...
export default function GroceryListPage() {
  ...
  // Compute totals for both flat and trip-based views
  const hasTrips = data?.trips && data.trips.length > 0;
  const flatSections = data?.sections ? getOrderedSections(data.sections) : [];

  const getAllItems = () => {
    if (hasTrips) {
      return data.trips.flatMap(t => Object.values(t.sections || {}).flat());
    }
    return flatSections.flatMap(sec => data.sections[sec] || []);
  };
  const totalUnchecked = getAllItems().filter(i => !checked[i.item]).length;
    ...
    <div>
      ...
      {data && (
        <>
          ...
          {/* Tip */}
          <p className="text-xs text-gray-400 mb-4 print:hidden">Tip: Tap an item to check it off as you shop. Takeout & leftover nights are excluded.</p>

          {/* Trip-based view */}
          {hasTrips && data.trips.map((trip, tripIdx) => {
            const tripSections = getOrderedSections(trip.sections);
            const tripUnchecked = tripSections.reduce((sum, sec) =>
              sum + (trip.sections[sec] || []).filter(i => !checked[i.item]).length, 0);

            return (
              <div key={trip.groceryDay} className={tripIdx > 0 ? 'mt-8' : ''}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">🛒</span>
                    <div>
                      <h3 className="text-lg font-bold text-gray-900">{trip.groceryDay} Shopping Trip</h3>
                      <p className="text-xs text-gray-500">
                        Meals for {trip.coversDays.join(', ')} — <strong>{tripUnchecked}</strong> items remaining
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {tripSections.length === 0 && (
                  <div className="card p-6 text-center text-gray-400 text-sm">
                    No groceries needed for this trip (takeout/leftover nights only).
                  </div>
                )}
              </div>
            );
          ...
          {/* Flat view (no grocery days configured) */}
          {!hasTrips && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
