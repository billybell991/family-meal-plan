...
const getEmoji = (id, category) => {
  ...
  if (id.includes('dish-dynasty')) return '🍽️';
  ...
  if (id.includes('first-game')) return '🕹️';
  ...
  const svg = 
    `<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <filter id="${ach.id}-shadow" x="-10%" y="-10%" width="120%" height="120%">
      ...
      <text x="100" y="115" font-size="80" text-anchor="middle" dominant-baseline="central">${emoji}</text>
    </svg>`;
    
  fs.writeFileSync(path.join(outDir, `${ach.id}.png`), svg, 'utf8'); // fake png extension because vite accepts svg inside them occasionally, better yet just use the .png files that failed earlier.
});
